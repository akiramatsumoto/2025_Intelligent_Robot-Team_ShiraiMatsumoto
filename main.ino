...
void loop() {
  ...
  int sensor_value_L2 = analogRead(LINE_CH3_PIN) >> 2;
  int sensor_value_R2 = analogRead(LINE_CH6_PIN) >> 2;

  int line_L = digitalRead(LINE_CH2_PIN);
  int line_R = digitalRead(LINE_CH7_PIN);

  int sensor_L_avr = (sensor_value_L + sensor_value_L2) / 2;
  int sensor_R_avr = (sensor_value_R + sensor_value_R2) / 2;

  int center = (sensor_L_avr + sensor_R_avr) / 2;
  float diff = (sensor_R_avr - center) - (sensor_L_avr - center);
  ...
  switch (state) {
    ...
    // 白井ここ書いて
    //0616_白井追加
      pidControl(sensor_L_avr, sensor_R_avr);
...
//0616_白井追加
void pidControl(int sensor_L_avr, int sensor_R_avr) {
  int center = (sensor_L_avr + sensor_R_avr) / 2;
  float diff = (sensor_R_avr - center) - (sensor_L_avr - center);
